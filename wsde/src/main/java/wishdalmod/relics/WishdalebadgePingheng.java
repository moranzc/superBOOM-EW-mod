...
public class WishdalebadgePingheng extends CustomRelic {
    public static final String ID = ModHelper.makePath("WishdalebadgePingheng");
    ...
    public WishdalebadgePingheng() {
        super(ID, ImageMaster.loadImage(IMG), ImageMaster.loadImage(IMG_OTL),
                AbstractRelic.RelicTier.STARTER, AbstractRelic.LandingSound.CLINK);
    }
    public String getUpdatedDescription() {
        return this.DESCRIPTIONS[0];
    }
    public CustomRelic makeCopy() {
        return new WishdalebadgePingheng();
    }
    ...
    public void onUseCard(AbstractCard card, UseCardAction action) {
        if (card.type == AbstractCard.CardType.ATTACK && this.activated) {
            ...
            AbstractDungeon.actionManager.addCardQueueItem(
                    new CardQueueItem(tmp, (AbstractMonster) target, card.energyOnUse, true, true),
                    true
            );
