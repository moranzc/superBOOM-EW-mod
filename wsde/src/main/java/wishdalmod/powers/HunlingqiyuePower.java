...
public class HunlingqiyuePower extends AbstractPower {
    public static final String POWER_ID = ModHelper.makePath("HunlingqiyuePower");
    ...
    private final Color flameColor = new Color(0.6f, 0.0f, 0.8f, 0.7f); // 紫色(byd这颜色真难调明明是紫色，但还是绿色)
    public HunlingqiyuePower(AbstractCreature owner) {
        this.name = NAME;
        this.ID = POWER_ID;
        this.owner = owner;
    ...
    public void update(int slot) {
        ...
        if (this.fireTimer >= 0.1f) {
            this.fireTimer = 0.0f;
            for (int i = 0; i < 4; i++) {
                AbstractDungeon.effectsQueue.add(new FireBurstParticleEffect(Settings.WIDTH * 0.1f + (float)Math.random() * Settings.WIDTH * 0.8f, 50.0f * Settings.scale
                ));
            }
        }
    }
